...
//код позволяет заполнить квадратную матрицу произвольного размера
...
int[,] SpiralArray(int n)
{
    ...
    int q = 0; // переменная позвоялет учитывать, что в определенный момент мы должны уменьшить на единицу радиус спирали
    ...
    while (true)
    {
        while (u < n - q)
        {
            if (n * n < count) return array; // как только количество помещенных в массив элементов превысит максимально допустимое, возвращаем массив
